...
function reducer(state: State, action: Action): State {
    switch (action.type) {
        ...
        case 'DEALER_DONE': {
            ...
            const lastResult = { outcome: comp.result as any, player: comp.player, dealer: comp.dealer, delta };
...
export default function GamePage() {
    ...
    // Sync chips with user on mount and when user chips change
    useEffect(() => {
        if (user) {
    ...
    // Update database when chips change at round end
    useEffect(() => {
        if (user && token && state.status === 'round_end' && state.lastResult) {
            updateChipsInDatabase(state.chips);
        }
    }, [state.status]);

    const updateChipsInDatabase = async (chips: number) => {
        try {
            await fetch('/api/update-chips', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ token, chips }),
            });
            // Update chips in context immediately
            updateChips(chips);
        } catch (error) {
    ...
    const placeBet = (bet: number) => {
        ...
        // Update chips in context immediately after placing bet
        if (user && token) {
            const newChips = state.chips - bet;
            updateChips(newChips);
            updateChipsInDatabase(newChips);
        }
    };
            ...
            <div className="h-screen flex items-center justify-center">
                <div className="p-4 flex flex-col items-center gap-6">
                    ...
                    <div className="flex flex-col items-center gap-6">
                        <div className="flex flex-col items-center">
                            <div className="flex gap-3">
                                {state.dealer.map(c => <CardView key={c.id} card={c} />)}
                            </div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-6 items-center justify-center">
                        ...
                        {state.status === 'idle' && <button className="px-3 py-2 rounded bg-white text-black border hover:bg-gray-200 transition duration-200" disabled={!state.currentBet} onClick={startHand}>Deal</button>}
